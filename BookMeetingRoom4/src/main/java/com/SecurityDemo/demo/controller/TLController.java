...
String role1 = user.getRoles().toString();
	...
	public String allDeptReqPm(Model m) {
		...
		List<Dept_request> deptRequest = deptRequestService.userAllDeptReq(user.getDepartment());
	...
	public String allMailRequest(Model m) {
		...
		List<mail_request> mail = mailRequestService.listAllMailuser(user.getDepartment());
		if(mail.isEmpty())
		{
